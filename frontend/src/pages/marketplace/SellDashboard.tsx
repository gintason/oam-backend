...
export default function SellDashboard() {
  ...
  const supportedCcy = pricing.data?.supported_currencies ?? ["NGN"];
  const { currency: displayCcy } = useCurrency();
  const [payCcy, setPayCcy] = useState<string>("NGN");
  useEffect(() => {
    setPayCcy(supportedCcy.includes(displayCcy.code) ? displayCcy.code : "NGN");
  }, [pricing.data, displayCcy.code]);
  function tierPrice(key: string): string {
    const p = pricing.data?.subscription?.[key]?.[payCcy];
    const n = p != null ? Number(p) : SELLER_TIERS.find((s) => s.key === key)?.price ?? 0;
    return `${CURRENCIES[payCcy as CurrencyCode]?.symbol ?? ""}${n.toLocaleString()}`;
  }
      ...
      <main className="mx-auto max-w-3xl px-4 py-5 sm:px-5 sm:py-6">
        ...
        {currentTier !== "pro" && (
          <Card className="mt-4 p-5">
            ...
            {supportedCcy.length > 1 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {supportedCcy.map((c) => (
                  <button
                    key={c}
                    onClick={() => setPayCcy(c)}
                    className={`h-8 rounded-full border px-3 text-[12.5px] font-medium transition ${payCcy === c ? "border-brand-green bg-brand-green/10 text-brand-green" : "border-hairline bg-paper text-ink hover:bg-mist"}`}
                  >
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-0.5 tabular text-[17px] font-bold text-ink">
                      {tier.price === 0 ? t("marketplace.tiers.free.label") : tierPrice(tier.key)}
