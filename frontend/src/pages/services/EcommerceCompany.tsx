    ...
    return (
      <div className="min-h-screen bg-mist">
        <main className="mx-auto max-w-3xl px-5 py-10 text-center">
          <p className="rounded-xl border border-hairline bg-paper p-6 text-[14px] text-muted">{t("ecommerce.storeNotFound")}</p>
        </main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-mist">
      <main className="mx-auto max-w-3xl px-5 py-8 sm:px-6">
        ...
        >
          <ArrowLeft size={15} strokeWidth={1.75} /> {t("ecommerce.allStores")}
        </button>
          ...
          <div className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[13.5px] text-muted">{t(`ecommerce.partners.${p.slug}.tagline`, p.tagline)}</p>
              </div>
            </div>
            <p className="mt-4 text-[14px] leading-relaxed text-ink">{t(`ecommerce.partners.${p.slug}.blurb`, p.blurb)}</p>
            ...
            {pending ? (
              <div className="mt-5 flex items-center gap-2 rounded-xl border border-warn/30 bg-warn/5 px-4 py-3 text-[13px] text-ink">
                ...
                {t("ecommerce.pending", { name: p.name })}
              </div>
            ) : (
              <button
                onClick={open}
              ...
              >
                <ShoppingCart size={17} strokeWidth={1.75} /> {t("ecommerce.shopOn", { name: p.name })}
        ...
        {/* Categories */}
        <h2 className="mb-3 mt-8 font-display text-[16px] font-semibold text-ink">{t("ecommerce.popularCategories")}</h2>
